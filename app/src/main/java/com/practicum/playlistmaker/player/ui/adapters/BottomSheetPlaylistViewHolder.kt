...
class BottomSheetPlaylistViewHolder(private val binding: BottomSheetViewBinding) : RecyclerView.ViewHolder(binding.root) {

    fun bind(playlist: Playlist, clickListener:((Int, List<Long>, Playlist) -> Unit)?) {
        with(binding) {
            playlistNameBottomSheet.text = playlist.playlistName
            tracksCountBottomSheet.text = playlist.tracksCount.toString() + " " + TrackWordConvertor.getTrackCountString(playlist.tracksCount, itemView)

        }
        Glide.with(itemView).load(playlist.uri).centerCrop()
            .placeholder(R.drawable.placeholder).into(binding.bottomSheetTrackCover)
